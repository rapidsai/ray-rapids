...
def make_blobs(
    ...
    actors : list of Ray RAFT Actors
        list of Ray RAFT Actors
    n_samples : int
        number of rows
    n_features : int
        number of features
    ...
    n_parts = len(actors)  # We currently work with one part per worker
    ...
    seeds = generator.randint(n_samples, size=n_parts)

    res = [
        actors[idx].execute.remote(
            _create_local_data,
            worker_row,
            n_features,
            centers,
